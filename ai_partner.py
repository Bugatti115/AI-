...
#python -m streamlit run ai_partner.py
#设置页面配置项
st.set_page_config(
    page_title="AI助手",
    page_icon="❤️",
...
#系统提示词
system_prompt = "你是一个智能助手，名字是%s,请回答用户问题。"

#初始化聊天信息
if "messages" not in st.session_state: #{"role":"user", "content":prompt}
    st.session_state.messages = []
#昵称
...
# 创建与AI大模型交互的客户端对象(DEEPSEEK_API_KEY环境变量)
client = OpenAI(
...
with st.sidebar:
    ...
    if st.button("新建会话",width = "stretch",icon = "✏️"):
        ...
        #2.创建新的会话
        if st.session_state.messages: #聊天信息不为空,True;否则 False
            st.session_state.messages = []
            st.session_state.current_session = generate_session_name()
            save_session()
            st.rerun ()  #重新运行当前页面
